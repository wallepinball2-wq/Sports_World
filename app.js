// ================================================================
// LEAGUE DISPLAY NAMES — randomized on each new save so no real
// league names ever appear. Internal code keys (state.ahl, cpuAHL,
// _league, etc.) are unchanged — only display strings use this.
// On load, state._leagueNames is restored so names stay consistent.
// ================================================================

const _CUP_NAMES = [
  'Founders Cup','Continental Cup','Alliance Cup','Summit Cup','Apex Cup',
  'Premier Cup','Meridian Cup','Legacy Cup','Pinnacle Cup','Sovereign Cup',
  'Vanguard Cup','Prestige Cup','Capital Cup','Frontier Cup','Triumph Cup',
];

const _PRO_LEAGUE_NAMES   = ['PHL','CHL','IHL','NLA','FHL','XHL','UHL','GHL','WHL','VHL'];
const _MINOR_LEAGUE_NAMES = ['NARL','NAAHL','ECAL','ACHL','AMHL','MARL','NCHL','PCHL','RARL','UCHL'];
const _LOW_LEAGUE_NAMES   = ['ECRL','LCHL','SCHL','DCHL','FCRL','GCRL','HCRL','JCRL','RCRL','XCRL'];

function _pickLeagueName(pool, exclude){
  const available = pool.filter(n => !exclude.includes(n));
  return available[Math.floor(Math.random() * available.length)];
}

function generateLeagueDisplayNames(){
  const pro   = _pickLeagueName(_PRO_LEAGUE_NAMES, []);
  const minor = _pickLeagueName(_MINOR_LEAGUE_NAMES, [pro]);
  const low   = _pickLeagueName(_LOW_LEAGUE_NAMES, [pro, minor]);
  const cup = _CUP_NAMES[Math.floor(Math.random() * _CUP_NAMES.length)];
  return { pro, minor, low, cup };
}

// Mutable object — values reassigned on new game or save load
const LEAGUE_NAMES = { pro: 'PHL', minor: 'NARL', low: 'ECRL', cup: 'Founders Cup' };

function applyLeagueNames(names){
  LEAGUE_NAMES.pro   = names.pro;
  LEAGUE_NAMES.minor = names.minor;
  LEAGUE_NAMES.low   = names.low;
  LEAGUE_NAMES.cup   = names.cup || LEAGUE_NAMES.cup;
}
...
const OVR_TIER_DEFAULTS = [
  ...
  { min:50, color:'#e67e22', label:'Minor'      },
...
function initState(teamName, previewTeam){
  ...
  // Randomize league display names for this save
  applyLeagueNames(generateLeagueDisplayNames());
  state = {
...
function renderTrade(){
  ...
  function playerRowHtml(p, onclick, showBlock){
    ...
    const leagueColors = { [LEAGUE_NAMES.minor]:'#5dade2', [LEAGUE_NAMES.low]:'#f39c12' };
...
function archivePlayerSeason(p, season, teamName, leagueName){
  if(!p.stats) return;
  if(!p.seasonHistory) p.seasonHistory = [];
  if(!p.careerTotals) p.careerTotals = freshCareerTotals(p.pos);

  // Build season record
  const record = { season, team: teamName, league: leagueName || LEAGUE_NAMES.pro, ...p.stats };
...
function archiveTeamSeasonStats(team, season, leagueName){
  if(!team || !team.roster) return;
  team.roster.forEach(p => archivePlayerSeason(p, season, team.name, leagueName));
}
...
function snapshotMidSeasonStats(p, oldTeamName, leagueName){
  if(!p || !p.stats) return;
  ...
  const record = { season, team: oldTeamName, league: leagueName || LEAGUE_NAMES.pro, traded: true, ...p.stats };
  p.seasonHistory.push(record);
  Object.keys(p.careerTotals).forEach(key => {
    p.careerTotals[key] = (p.careerTotals[key] || 0) + (p.stats[key] || 0);
  });
...
function _autoSimTick(){
  ...
  // Simmable phases — PLAYOFFS is always included; individual stop checks handle halting
  const simmablePhases = [PHASES.PRESEASON, PHASES.REGULAR_SEASON, PHASES.TRADE_DEADLINE, PHASES.PLAYOFFS];
  if(!simmablePhases.includes(phase)){
  ...
  if(cfg.stopOnSeasonEnd && phase !== PHASES.PLAYOFFS && isOnOrAfterDate(cal, PHASE_DATES.regularSeasonEnd.month, PHASE_DATES.regularSeasonEnd.day)){ stopAutoSim('Regular Season has ended.'); return; }
...
function autoSimCheckPlayoffSeries(){
  ...
  const cfg = getAutoSimSettings();
  // If targeting draft, don't stop on playoff series — sim straight through
...
function autoSimCheckSeasonEnd(){
  const cfg = getAutoSimSettings();
  // If targeting draft, don't stop at season end — keep simming through playoffs
  if((cfg.stopOnSeasonEnd || cfg.target === 'season-end') && state.calendar?.phase !== PHASES.PLAYOFFS && _autoSimInterval !== null){
...
function simToDate(targetMonthIdx, targetDay){
  ...
  if(targetWeek <= cal.week){
    showFlash('Current or past week', 'Use the calendar to click a future day to sim to it.', 'otl');
    return;
  }
...
function advancePlayoffs(){
  ...
  if(currentRound.length === 1 && allDone){
    ...
    state.log.push(`🏆 ${bracket.champion} wins the ${LEAGUE_NAMES.cup}!`);
    renderPlayoffs();
    return;
  }
...
function calCheckAdvanceRound(){
  ...
  if(currentRound.length === 1){
    bracket.champion = currentRound[0].winner;
    showFlash('🏆 ' + LEAGUE_NAMES.cup + ' Champion!', bracket.champion + ' wins!', 'win');
    state.log.push(`🏆 ${bracket.champion} wins the ${LEAGUE_NAMES.cup}!`);
    // Immediately transition phase so auto-sim stops cleanly at draft
    if(state.calendar && state.calendar.phase === PHASES.PLAYOFFS){
      state.calendar.phase = PHASES.DRAFT;
      state.log.push('📋 Entry Draft is underway.');
      showOffseasonTabs();
      showTab('draft');
      autoSimCheckDraft();
...
function renderPlayoffs(){
  ...
  if(bracket.champion){
    champHtml = `<div style="text-align:center;padding:16px 12px 20px;background:linear-gradient(135deg,rgba(243,156,18,0.08),rgba(241,196,15,0.04));border:1px solid rgba(243,156,18,0.3);border-radius:10px;margin-bottom:20px;">
      ...
      <div style="font-size:12px;color:var(--text2);letter-spacing:1px;text-transform:uppercase;margin-top:4px;">${LEAGUE_NAMES.cup} Champions</div>
    </div>`;
  }

  // ── Round name labels ─────────────────────────────────────────────
  const ROUND_LABELS = ['First Round','Second Round','Conf. Finals',LEAGUE_NAMES.cup + ' Final'];
...
function sendToAHL(playerId){
  ...
  console.log(`[Send Down] ${p.name} (OVR ${p.ovr}) → ${LEAGUE_NAMES.minor} | Reason: ${reason}${warning?' | '+warning:''}`);
  ...
  state.ahl.roster.push(p);
  autoSetLines();
  state.log.push(`⬇️ ${p.name} (${p.ovr} OVR) sent to ${LEAGUE_NAMES.minor}`);
  renderAll();
  renderAffiliates();
}
...
function sendToECHL(playerId){
  ...
  if(idx!==-1){
    ...
    console.log(`[Send Down] ${p.name} (OVR ${p.ovr}) → ${LEAGUE_NAMES.low} | From ${LEAGUE_NAMES.pro} roster`);
    state.myTeam.roster.splice(idx, 1);
    p._affiliate = 'echl';
    // Always preserve the NHL salary so callup can restore it
    if(!p.nhlSalary) p.nhlSalary = p.salary;
    ...
    state.echl.roster.push(p);
    state.log.push(`⬇️ ${p.name} (${p.ovr} OVR) sent to ${LEAGUE_NAMES.low}`);
  } else {
    // Demoting from AHL to ECHL
    const idx2 = state.ahl.roster.findIndex(p=>p.id===playerId);
    if(idx2!==-1){
      const p = state.ahl.roster[idx2];
      console.log(`[Send Down] ${p.name} (OVR ${p.ovr}) → ${LEAGUE_NAMES.low} | From ${LEAGUE_NAMES.minor} roster`);
...
function autoTrimRoster(){
  ...
  toSend.forEach(p=>{
    if(p.ovr < AHL_MIN){
      console.log(`[Auto Trim] ${p.name} (OVR ${p.ovr}, ${p.pos}) sent to ${LEAGUE_NAMES.low} — below ${LEAGUE_NAMES.minor} threshold`);
      sendToECHL(p.id);
    } else {
      console.log(`[Auto Trim] ${p.name} (OVR ${p.ovr}, ${p.pos}) sent to ${LEAGUE_NAMES.minor} — roster over limit`);
      sendToAHL(p.id);
    }
  });
}
...
function callUpFromECHLToAHL(playerId){
  ...
  repairPromotedContract(p);
  state.ahl.roster.push(p);
  state.log.push(`⬆️ ${p.name} promoted to ${LEAGUE_NAMES.minor} (${state.ahl.name})`);
  renderAll();
  renderAffiliates();
}
...
function renderAffiliates(){ if(!gameStarted) return;
    ...
    return `<table width="100%" style="border-collapse:collapse;font-size:13px;margin-top:8px;">
      ...
      ${aff.roster.map(p=>`<tr>
        ...
        <td style="padding:7px 8px;border-bottom:1px solid rgba(100,160,220,0.07);">
          <button class="btn btn-xs btn-gold" onclick="callUp('${p.id}','${fromKey}')">↑ ${LEAGUE_NAMES.pro}</button>
          ${fromKey==='echl'?`<button class="btn btn-xs" style="margin-left:4px;" onclick="callUpFromECHLToAHL('${p.id}')">↑ ${LEAGUE_NAMES.minor}</button>`:''}
          ${fromKey==='ahl'?`<button class="btn btn-xs" style="margin-left:4px;border-color:rgba(192,57,43,0.4);color:var(--red2);" onclick="sendToECHL('${p.id}')">↓ ${LEAGUE_NAMES.low}</button>`:''}
        </td>
      </tr>`).join('')}
      </tbody></table>`;
  }
  ...
  html += `<div class="affiliate-section">
    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:12px;">
      <div>
        <div class="affiliate-header" style="color:#5dade2;">${LEAGUE_NAMES.minor} — ${state.ahl.name}</div>
  ...
  html += `<div class="affiliate-section">
    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:12px;">
      <div>
        <div class="affiliate-header" style="color:var(--gold);">${LEAGUE_NAMES.low} — ${state.echl.name}</div>
  ...
  html += `<div class="affiliate-section">
    <div class="affiliate-header" style="font-size:15px;margin-bottom:8px;">Send Down from ${LEAGUE_NAMES.pro} Roster</div>
    <div style="font-size:13px;color:var(--text2);margin-bottom:12px;">Players on your ${LEAGUE_NAMES.pro} roster you can assign to affiliates.</div>
    <table width="100%" style="border-collapse:collapse;font-size:13px;">
      <thead><tr>
        <th style="text-align:left;padding:6px 8px;font-size:12px;color:var(--text2);border-bottom:1px solid var(--border);">Player</th>
        <th style="text-align:left;padding:6px 8px;font-size:12px;color:var(--text2);border-bottom:1px solid var(--border);">Pos</th>
      ...
      ${getDemotionCandidates(state.myTeam.roster).map(p=>{
        ...
        const ahlLabel = exempt ? '&#8595; ' + LEAGUE_NAMES.minor : '&#128203;&rarr; ' + LEAGUE_NAMES.minor;
        const echlLabel = exempt ? '&#8595; ' + LEAGUE_NAMES.low : '&#128203;&rarr; ' + LEAGUE_NAMES.low;
        const ahlTitle = exempt ? 'Waiver-exempt: direct ' + LEAGUE_NAMES.minor : 'Waiver required';
        const echlTitle = exempt ? 'Waiver-exempt: direct ' + LEAGUE_NAMES.low : 'Waiver required';
        return `<tr>
...
`;
  ...
  if(playerPageTab==='history'){
    ...
    if(!history.length && !hasCurrentStats){
      ...
      if(isGoalie){
        ...
        html += `<table width="100%" style="border-collapse:collapse;font-size:13px;">
          <thead><tr style="border-bottom:1px solid var(--border);">
            ${['Season','Lg','Team','GP','W','L','GA','SV%','GAA'].map(h=>`<th style="padding:7px 10px;text-align:left;font-size:11px;color:var(--text2);font-weight:700;text-transform:uppercase;letter-spacing:0.8px;">${h}</th>`).join('')}
          </tr></thead><tbody>
          ${[...history].map(s=>{
            const sv = s.sa>0 ? (s.saves/s.sa*100).toFixed(1)+'%' : '—';
            const gaa = s.gp>0 ? (s.ga/s.gp).toFixed(2) : '—';
            const lgLabel = s.league || LEAGUE_NAMES.pro;
            return `<tr style="border-bottom:1px solid rgba(100,160,220,0.07);">
              <td style="padding:7px 10px;font-weight:600;">${s.season}</td>
              <td style="padding:7px 10px;font-family:'Barlow Condensed',sans-serif;font-size:11px;font-weight:700;color:var(--text2);">${lgLabel}</td>
          ...
          ${hasCurrentStats ? `<tr style="border-bottom:1px solid rgba(100,160,220,0.07);background:rgba(255,255,255,0.04);">
            ...
            <td style="padding:7px 10px;font-family:'Barlow Condensed',sans-serif;font-size:11px;font-weight:700;color:var(--text2);">${LEAGUE_NAMES.pro}</td>
        ...
        html += `<table width="100%" style="border-collapse:collapse;font-size:13px;">
          <thead><tr style="border-bottom:1px solid var(--border);">
            ${['Season','Lg','Team','GP','G','A','PTS','+/-'].map(h=>`<th style="padding:7px 10px;text-align:left;font-size:11px;color:var(--text2);font-weight:700;text-transform:uppercase;letter-spacing:0.8px;">${h}</th>`).join('')}
          </tr></thead><tbody>
          ${[...history].map(s=>{
  ...
  html += `<div style="margin-top:20px;display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
    ${isFA ? `<button class="btn btn-gold" onclick="openSign('${p.id}');closePlayerPage();">Make Offer</button>` : isOtherTeam
      ...
      <button class="btn" onclick="sendToAHL('${p.id}');closePlayerPage();">↓ Send to ${LEAGUE_NAMES.minor}</button>
...
function renderCalendar(){ if(!gameStarted) return;
  ...
  if(state.playoffsStarted && state.bracket){
    ...
    if(upcomingPoList.length){
      const roundNames2 = ['First Round','Second Round','Conference Finals',LEAGUE_NAMES.cup + ' Final'];
  ...
  if(state.playoffsStarted && state.bracket && viewMonth >= 6){
    ...
    if(mySeries.length){
      const roundNames = ['First Round','Second Round','Conference Finals',LEAGUE_NAMES.cup + ' Final'];
...
function renderContracts(){ if(!gameStarted) return;
  ...
  function affiliateContractRow(p, fromLeague){
    ...
    const salDisplay = p.isTwoWay && nhlSal
      ? `<span style="font-weight:600;color:#5dade2;">$${curSal.toFixed(2)}M</span> <span style="font-size:10px;color:var(--text2);">↑${LEAGUE_NAMES.pro} $${nhlSal.toFixed(2)}M</span>`
      : `<span style="font-weight:600;color:var(--text2);">$${curSal.toFixed(2)}M</span>`;
    const leagueTag = fromLeague === 'ahl'
      ? `<span style="font-size:10px;font-family:'Barlow Condensed',sans-serif;font-weight:700;padding:1px 6px;border-radius:3px;background:rgba(93,173,226,0.15);color:#5dade2;border:1px solid rgba(93,173,226,0.3);">${LEAGUE_NAMES.minor}</span>`
      : `<span style="font-size:10px;font-family:'Barlow Condensed',sans-serif;font-weight:700;padding:1px 6px;border-radius:3px;background:rgba(243,156,18,0.12);color:var(--gold);border:1px solid rgba(243,156,18,0.3);">${LEAGUE_NAMES.low}</span>`;
    return `<tr>
      ...
      <td style="${tdS}">
        <div style="display:flex;gap:5px;">
          <button class="btn btn-xs btn-gold" onclick="callUp('${p.id}','${fromLeague}');renderContracts();">↑ ${LEAGUE_NAMES.pro}</button>
          ${fromLeague==='ahl'?`<button class="btn btn-xs" style="color:var(--red2);border-color:rgba(192,57,43,0.3);" onclick="sendToECHL('${p.id}');renderContracts();">↓ ${LEAGUE_NAMES.low}</button>`:''}
          ${fromLeague==='echl'?`<button class="btn btn-xs" onclick="callUpFromECHLToAHL('${p.id}');renderContracts();">↑ ${LEAGUE_NAMES.minor}</button>`:''}
        </div>
      </td>
    </tr>`;
  }
  ...
  html += `<div style="margin-bottom:20px;">
    <div style="font-family:'Barlow Condensed',sans-serif;font-size:13px;font-weight:700;color:var(--text2);text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;">
      🏒 ${LEAGUE_NAMES.pro} — Active Contracts (${locked.length})
    </div>
  ...
  if(expiringNext.length){
    html += `<div style="margin-bottom:20px;">
      <div style="font-family:'Barlow Condensed',sans-serif;font-size:13px;font-weight:700;color:var(--red2);text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;">
        ⚠️ ${LEAGUE_NAMES.pro} — Expiring After This Season (${expiringNext.length})
      </div>
      <div style="overflow-x:auto;">
      <table width="100%" style="border-collapse:collapse;">
        <thead>${colHeaders}</thead>
    ...
    html += `<div style="margin-bottom:20px;">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
        <div style="font-family:'Barlow Condensed',sans-serif;font-size:13px;font-weight:700;color:#5dade2;text-transform:uppercase;letter-spacing:1px;">
          ${LEAGUE_NAMES.minor} — ${state.ahl.name} (${ahlRoster.length} players)
        </div>
        <div style="font-size:11px;color:var(--text2);">Minor league salary counts against cap only when on ${LEAGUE_NAMES.pro} roster</div>
      </div>
      <div style="overflow-x:auto;">
      <table width="100%" style="border-collapse:collapse;">
        <thead>${colHeaders}</thead>
    ...
    html += `<div style="margin-bottom:20px;">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
        <div style="font-family:'Barlow Condensed',sans-serif;font-size:13px;font-weight:700;color:var(--gold);text-transform:uppercase;letter-spacing:1px;">
          ${LEAGUE_NAMES.low} — ${state.echl.name} (${echlRoster.length} players)
        </div>
      </div>
      <div style="overflow-x:auto;">
      <table width="100%" style="border-collapse:collapse;">
...
function saveGame(slotId){
  try {
    ...
    state._leagueNames  = { ...LEAGUE_NAMES };
...
function loadGameById(id){
  try {
    ...
    if(state._leagueNames) applyLeagueNames(state._leagueNames);
    currentSaveId = id;
    gameStarted = true;
    renderAll();
...
function placeOnWaivers(playerId, targetLeague){
  ...
  const idx = state.myTeam.roster.findIndex(p => p.id === playerId);
  if(idx === -1){ alert(`Player not found on ${LEAGUE_NAMES.pro} roster.`); return; }
  const p = state.myTeam.roster[idx];
  ...
  if(isWaiverExempt(p)){
    ...
    state.log.push(`📋 ${p.name} is waiver-exempt — assigned directly to ${targetLeague === 'echl' ? LEAGUE_NAMES.low : LEAGUE_NAMES.minor}.`);
    renderAll();
    renderAffiliates();
    return;
  }
    ...
    if(!claimed){
      ...
      if(entry.isCpuOriginated){
        ...
        if(state.echl.roster.length < ECHL_MAX){
          ...
          state.echl.roster.push(p);
          state.log.push(`✅ ${p.name} cleared waivers — assigned to ${LEAGUE_NAMES.low}.`);
        } else {
          p._affiliate = 'ahl';
          if(p.isTwoWay && p.minorSalary != null){
            p.nhlSalary = p.salary;
          ...
          state.ahl.roster.push(p);
          state.log.push(`✅ ${p.name} cleared waivers — ${LEAGUE_NAMES.low} full, assigned to ${LEAGUE_NAMES.minor}.`);
        }
      } else {
        ...
        if(state.ahl.roster.length < AHL_MAX){
          ...
          state.ahl.roster.push(p);
          state.log.push(`✅ ${p.name} cleared waivers — assigned to ${LEAGUE_NAMES.minor}.`);
          showFlash('Waivers Cleared', `${p.name} cleared waivers and is now in the ${LEAGUE_NAMES.minor}.`, 'win');
        } else {
          p._affiliate = 'echl';
          if(p.isTwoWay && p.minorSalary != null){
            p.nhlSalary = p.salary;
          ...
          state.echl.roster.push(p);
          state.log.push(`✅ ${p.name} cleared waivers — ${LEAGUE_NAMES.minor} full, assigned to ${LEAGUE_NAMES.low}.`);
          showFlash('Waivers Cleared', `${p.name} cleared waivers — ${LEAGUE_NAMES.minor} full, sent to ${LEAGUE_NAMES.low}.`, 'win');
        }
      }
      autoSetLines();
    }
...
function claimFromWaivers(entryIdx){
  ...
  if(state.myTeam.roster.length >= 23){
    alert(`${LEAGUE_NAMES.pro} roster is full (23 players). Make room before claiming.`);
    return;
  }
...
function renderWaivers(){
  ...
  html += `<div class="affiliate-section" style="margin-bottom:20px;">
    ...
    <p style="font-size:13px;color:var(--text2);margin-bottom:12px;">
      ...
      <span style="color:var(--gold);">⭐ Exempt</span> players skip waivers (young / low ${LEAGUE_NAMES.pro} games played).
    </p>`;

  if(sendDownCandidates.length === 0){
    html += `<p style="font-size:13px;color:var(--text2);padding:8px 0;">No demotable players on your ${LEAGUE_NAMES.pro} roster.</p>`;
  } else {
    html += `<table width="100%" style="border-collapse:collapse;font-size:13px;">
      <thead><tr>
        <th style="text-align:left;padding:6px 8px;font-size:12px;color:var(--text2);border-bottom:1px solid var(--border);">Player</th>
    ...
    sendDownCandidates.forEach(p => {
      ...
      html += `<tr>
        ...
        <td style="padding:7px 8px;border-bottom:1px solid rgba(100,160,220,0.07);">
          <button class="btn btn-xs" onclick="placeOnWaivers('${p.id}','ahl')" title="${exempt?'Exempt — direct '+LEAGUE_NAMES.minor:'Place on waivers → '+LEAGUE_NAMES.minor}">
            ${exempt ? '↓ ' + LEAGUE_NAMES.minor : '📋 Waivers → ' + LEAGUE_NAMES.minor}
          </button>
          <button class="btn btn-xs" style="margin-left:4px;" onclick="placeOnWaivers('${p.id}','echl')" title="${exempt?'Exempt — direct '+LEAGUE_NAMES.low:'Place on waivers → '+LEAGUE_NAMES.low}">
            ${exempt ? '↓ ' + LEAGUE_NAMES.low : '📋 Waivers → ' + LEAGUE_NAMES.low}
          </button>
        </td>
      </tr>`;
    });
  ...
  if(recent.length > 0){
    ...
    recent.forEach(entry => {
      ...
      const result = entry.claimedBy
        ...
        : `<span style="color:#2ecc71;">Cleared → ${entry.targetLeague === 'echl' ? LEAGUE_NAMES.low : LEAGUE_NAMES.minor}</span>`;
      html += `<tr>
  ...
  html += `<div style="margin-top:20px;padding:12px;background:rgba(255,255,255,0.03);border:1px solid var(--border);border-radius:6px;font-size:12px;color:var(--text2);">
    ...
    Skaters are waiver-exempt until they've played <strong style="color:var(--text);">${WAIVER_EXEMPT_SKATER_GP} ${LEAGUE_NAMES.pro} games</strong> 
    or turn <strong style="color:var(--text);">25</strong> (whichever comes first). 
    Goalies are exempt until <strong style="color:var(--text);">${WAIVER_EXEMPT_GOALIE_GP} ${LEAGUE_NAMES.pro} games</strong>. 
